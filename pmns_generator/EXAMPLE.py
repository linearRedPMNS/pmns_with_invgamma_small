...
alpha_max = 8	#maximum absolute value allowed for 'alpha'

lambda_max = 8	#maximum absolute value allowed for 'lambda'

delta = 0		#the (minimum) number of desired "free" additions before a modular multplication  

n = (p_size//phi_log2) + 1	#IMPORTANT: if the generator takes too long to find a PMNS, 'n' should be incremented as much as necessary, especially if "delta > 0" 

nb_pmns = 6	#the desired number of PMNS (if any)


pmns_list = look_for_good_primes(p_size, lv_tolerance, n, phi_log2, delta, alpha_max, lambda_max, double_spare, nb_pmns)	#THE PMNS GENERATOR
